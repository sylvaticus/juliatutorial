...
```
Also, please note that this action is automatically run over a certain interval. GitHub stops this kinds of cron-based actions for inactive repositories, sending a warning email to you to prevent it. So, until a more solid solution is found, check your email if your repository is no longer actively developed!
...
While `generate()` produce a _basic_ documentation configuration we are going to extend it a bit in order to have (a) multiple pages, (b) automatic inclusion of all documented items and (c ) hosting of the documentation in github.
...
We can add the two badges for the last version and development version of the documentation to the readme:
