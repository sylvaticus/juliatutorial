...
* Push an element to the end of a: `push!(a,b)` (as a single element even if it is an Array. Equivalent to python `append` or `+=`)
* To append all the elements of b to a: `append!(a,b)` (if b is a scalar obviously push! and append! are the same. Attention that a string is treated as a list!. Equivalent to Python `extend`)
